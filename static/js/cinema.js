window.onload = function(){
	...
	// var city = document.getElementsByClassName("js-geo-city")[0].innerHTML;
	// document.getElementsByClassName("shou")[0].innerHTML = city;
};
...
function load() {
	$(".movie").click(function () {
        ...
        $('.plist-container.active').removeClass('active');
        $('.show-list').eq($(this).index()).children('.show-date').children('.date-item').eq(0).addClass('active');
        $('.show-list').eq($(this).index()).children('.plist-container').eq(0).addClass('active');
	})
}

$(document).ready(function () {
    $('.date-item').eq(0).addClass('active');
    $('.plist-container').eq(0).addClass('active');
    /*时间切换*/
    $('.date-item').click(function () {
        console.log($(this).index());
        $('.show-list.active .date-item.active').removeClass('active');
        $(this).addClass('active');

        $('.show-list.active .plist-container.active').removeClass('active');
        $('.show-list.active .plist-container').eq($(this).index()-1).addClass('active');
    });
});